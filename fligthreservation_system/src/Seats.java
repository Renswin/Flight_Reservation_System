...
class Seats{
    ...
    private static List<Flight> listFlights = new ArrayList<>();
    ...
    public static void bookSeat(int index, Passenger passenger, String TicketType) {
        ...
        if (index < availableFlights.size()) {
            ...
            if (TicketType.equals("RegularTicket")) {
                ...
                System.out.println("Choose the services ...Food, Water,Scanks ");
                ...
                while (true) {
                    ...
                    if (!option)
                        ...
                        System.out.println("Choose the services ...Food,Water,Snacks ");
                ...
                ticket = new RegularTicket((long)(Math.random() * 10000), availableFlights.get(index), passenger, seatNumber,
                        2500, services);
                ticket.setStatus("Confirmed");

                int listIndex=listFlights.indexOf(availableFlights.get(index));
                int noOfSeatsBooked = listFlights.get(listIndex).getNoOfSeatsBooked();
                listFlights.get(listIndex).setNoOfSeatsBooked(noOfSeatsBooked + 1);
                ...
                Adress hotelAddress = new Adress(street, city, state);
                ...
                ticket = new TouristTicket((long)(Math.random() * 10000), availableFlights.get(index), passenger, seatNumber,
                        3500, hotelAddress, touristLocationsList);
    ...
    public static List<Flight> getAvailableFlights() {
        ...
        for (int i = 0; i < listFlights.size(); i++) {
            if (listFlights.get(i).getNoOfSeatsBooked() < listFlights.get(i).getCapacity()) {
                availableFlights.add(listFlights.get(i));
            }
        }
    ...
    public static String checkStatus(double PNRNumber, Passenger p) {
        List<Ticket> ticket = Seats.getAllTickets(p);
        for (Ticket t : ticket) {
            ...
            if (t.getPNRNumber() == pNRNumber) {
                if (t instanceof TouristTicket) {
                    switch (op2) {
                        ...
                        case 1:
                            ((TouristTicket) t).addLocation(location);
                            break;

                        case 2:
                            ((TouristTicket) t).removeLocation(location);
                            break;
                    }
                }
